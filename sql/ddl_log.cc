...
static void set_global_from_ddl_log_entry(const DDL_LOG_ENTRY *ddl_log_entry)
{
  ...
  int4store(file_entry_buf+DDL_LOG_ID_POS,  ddl_log_entry->unique_id);
  bzero(file_entry_buf+DDL_LOG_ID_POS+4,
        global_ddl_log.name_pos - DDL_LOG_ID_POS - 4);
                                          ...
                                          const uint read_entry)
{
  ...
  ddl_log_entry->unique_id=  uint4korr(file_entry_buf + DDL_LOG_ID_POS);
...
/*
  Get from and to db and table.
  If lower_case_table_names is set, to_converted_name is set to the lower case
  of to_tablename.
*/


static void get_db_and_table(DDL_LOG_ENTRY *ddl_log_entry,
                             LEX_CSTRING *from_db, LEX_CSTRING *from_table,
                             LEX_CSTRING *to_db,   LEX_CSTRING *to_table,
                             LEX_CSTRING *to_converted_name,
                             char *conv_path)
{
  ...
  *to_db=      ddl_log_entry->db;
  ...
  {
    uint errors;
    to_converted_name->str= conv_path;
...
/*
  ...
  If swap_tables == 0
    Convert triggers for from_db.from_table -> db.name
  else
    Convert triggers for db.name -> from_db.from_table
*/

static void rename_triggers(THD *thd, DDL_LOG_ENTRY *ddl_log_entry,
                            bool swap_tables)
{
  LEX_CSTRING from_table, to_table, from_db, to_db, to_converted_name;
  char from_path[FN_REFLEN+1], to_path[FN_REFLEN+1], conv_path[FN_REFLEN+1];

  if (!swap_tables)
    get_db_and_table(ddl_log_entry, &from_db, &from_table, &to_db, &to_table,
                     &to_converted_name, conv_path);
  else
    get_db_and_table(ddl_log_entry, &to_db, &to_table, &from_db, &from_table,
                     &to_converted_name, conv_path);
      ...
      */
      ...
      if (!access(from_path, F_OK))             // Does #sql-alter.. exists?
      {
        ...
        */
        ...
        {
          /* Table is up to date. Replace old .frm file with new one */
  ...
  case DDL_LOG_STORE_QUERY_ACTION:
  {
    ...
    */
    if (ddl_log_entry->flags)
    {
      ...
      */
      if (recovery_state.query.alloc(ddl_log_entry->unique_id+1))
        goto end;
      recovery_state.query.length(0);
    }
    if (unlikely(recovery_state.query.length() +
                 ddl_log_entry->tmp_name.length >
    ...
    recovery_state.query.qs_append(&ddl_log_entry->tmp_name);
    break;
  }
  default:
    DBUG_ASSERT(0);
                          ...
                          const LEX_CSTRING *query)
{
  ...
  ddl_log_entry.unique_id=    (uint32) frm_length;
  ddl_log_entry.db=           *const_cast<LEX_CSTRING*>(db);
  ddl_log_entry.name=         *const_cast<LEX_CSTRING*>(table);
                             ...
                             DDL_LOG_STATE *depending_state)
{
  ...
  ddl_log_entry.tmp_name=     *const_cast<LEX_CSTRING*>(path);
  if (depending_state)
    ddl_log_entry.unique_id= (uint32) depending_state->execute_entry->entry_pos;
  DBUG_RETURN(ddl_log_write(ddl_state, &ddl_log_entry));
}
                         ...
                         const char *query, size_t length)
{
  ...
  while (length)
  {
    ...
    ddl_log_entry.tmp_name.str= query;
    ddl_log_entry.tmp_name.length= write_length;
